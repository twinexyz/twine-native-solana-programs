...
use crate::{
    core::{
        ...
        state::{
            ForcedWithdrawMessagesBuffer, ForcedWithdrawMessagesReplicator, RoleType,
            TwineChainRoleManager, TwineChainStorage,
        },
    },
    utils::{
        address_derivation::{
            derive_forced_withdraw_message_buffer, derive_forced_withdraw_messages_replicator,
            derive_role_manager, derive_twine_chain_storage, verify_derived_address,
            verify_system_program,
        },
        constants::{
            FORCED_WITHDRAWAL_MEESSAGES_REPLICATOR_PREFIX, MAX_MESSAGE_NONCE, MESSAGE_NONCE_GAP,
        },
    },
};
...
pub fn copy_forced_withdrawal_pda(
    ...
    let forced_withdraw_messages_replicator_acc = next_account_info(account_info_iter)?;
    ...
    validate_accounts(
        ...
        forced_withdraw_messages_replicator_acc,
        role_manager_acc,
        initializer_acc,
        system_program,
    )?;
    ...
    let mut withdraw = ForcedWithdrawMessagesBuffer::deserialize(
    ...
    if forced_withdraw_messages_replicator_acc.data_is_empty() {
        let rent = Rent::default();
        let (_, forced_withdraw_messages_replicator_bump) =
            derive_forced_withdraw_messages_replicator(&program_id, start_nonce, end_nonce);
        let replicator_space = 1 + 8 + 8 + 8 + 4 + (MAX_MESSAGE_NONCE * 32);
        let required_lamports = rent.minimum_balance(replicator_space);
        let create_ix = system_instruction::create_account(
            initializer_acc.key,
            forced_withdraw_messages_replicator_acc.key,
            required_lamports,
        ...
        invoke_signed(
            ...
            &[
                ...
                forced_withdraw_messages_replicator_acc.clone(),
    ...
    let mut withdraw_messages_replicator_data = ForcedWithdrawMessagesReplicator::deserialize(
        &mut &forced_withdraw_messages_replicator_acc.data.borrow()[..],
    )
    .map_err(|_| ProgramError::InvalidAccountData)?;
...
fn validate_accounts(
    ...
    forced_withdraw_messages_replicator_acc: &AccountInfo,
    ...
    let (expected_deposit_pda, _) = derive_forced_withdraw_message_buffer(program_id);
    verify_derived_address(expected_deposit_pda, forced_withdraw_message_buffer_acc)?;

    let (expected_role_manager_pda, _) = derive_role_manager(program_id);

    verify_derived_address(expected_role_manager_pda, role_manager_acc)?;

    let (expected_withdraw_messages_replicator_pda, _) =
        derive_forced_withdraw_messages_replicator(program_id, start_nonce, end_nonce);
    verify_derived_address(
        expected_withdraw_messages_replicator_pda,
        forced_withdraw_messages_replicator_acc,
    )?;
