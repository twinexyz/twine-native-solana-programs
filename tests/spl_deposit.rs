...
use {
    helpers::{
        tokens_gateway_helper::{
            ...
            TokensGatewayAccounts,get_or_create_ata,
        }
    },
    solana_program_test::*,
    solana_sdk::{
...
async fn spl_token_deposit_succeed() {
    ...
    let (spl_token_pubkey, user_token_account) = create_spl_and_mint(&mut context,&accounts.chain_admin,9,100).await;
    let spl_token_vault = get_or_create_ata(&mut context,&accounts.chain_admin,&derive_spl_vault_authority().0,&spl_token_pubkey).await;
