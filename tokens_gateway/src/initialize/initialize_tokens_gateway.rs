...
mod test {
    ...
    fn test_initialize_tokens_gateway_success() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let mut spl_tokens_vault_data_lamports = rent
         ...
         let mut executed_payouts_buffer_lamports =
            rent.minimum_balance(8 + ExecutedPayoutsBuffer::SPACE);
        ...
        let mut executed_payouts_buffer_data = vec![0u8; 8 + ExecutedPayoutsBuffer::SPACE];
        ...
        let mut executed_payouts_buffer_owner = program_id;
        ...
        let executed_payouts_buffer_account = create_test_account_info(
            &executed_payouts_buffer_key,
            false,
            true,
            &mut executed_payouts_buffer_lamports,
            &mut executed_payouts_buffer_data,
            &mut executed_payouts_buffer_owner,
        );
        ...
        let accounts = vec![
            ...
            spl_tokens_vault_data_account.clone(),
             executed_payouts_buffer_account.clone(),
        ...
        let executed_payouts_data_ref = executed_payouts_buffer_account.data.borrow();
        let mut executed_payouts_data_slice = &executed_payouts_data_ref[..];
        let deserialized_executed_payouts =
            ExecutedPayoutsBuffer::deserialize(&mut executed_payouts_data_slice)?;
        assert!(
            deserialized_executed_payouts.is_initialized,
            "Executed withdrawals buffer should be initialized"
        );
        assert_eq!(
            deserialized_executed_payouts.payout_nonce_lower_bound, 0,
            "Withdrawal nonce lower bound should be 0"
        );
        assert!(
            deserialized_executed_payouts
