...
use crate::{
    core::{
        ...
        state::{
            ...
            TokenDecimalMappings,L2WithdrawValues
        },
    },
    utils::{
...
pub fn execute_native_l2_withdrawal(
    ...
    let actual_amount = TokenDecimalMappings::parse_amount_to_u64(&converted_amount)?;
        ...
        let mut executed_withdrawal_buffer = 
        ExecutedWithdrawalsBuffer::deserialize(&mut  &executed_withdrawals_buffer_acc.data.borrow()[..])
            .map_err(|_| ProgramError::InvalidAccountData)?;

        // For L2 initiated withdrawals
        if withdrawal_values.nonce
    ...
    msg!(
    "EVENT:NATIVE_L2_WITHDRAWAL_SUCCESSFUL: nonce={}, l1_receiver_address={}, l1_token_address={}, chain_id={}, amount={}, slot={}",
...
pub fn decode_l2_withdraw_values(bytes: &[u8]) -> Result<L2WithdrawValues, ProgramError> {
    ...
    let l1_token_address = decode_string_field(&bytes[92..124])?;
    let l2_token_address = decode_string_field(&bytes[124..166])?;
    let amount = decode_string_field(&bytes[166..])?;
