...
use tokens_gateway::{
    ...
    utils::address_derivation::{
        ...
        derive_spl_tokens_vault_data, derive_spl_vault_authority, derive_token_decimal_mappings,
    },
};
use twine_chain::{
...
pub fn get_all_pdas() -> Result<()> {
    ...
    let derive_detailed_messages_buffer = derive_detailed_messages_buffer(&twine_chain_id()).0;
    ...
    println!("Messasges Buffer PDA: {:?} ", messages_buffer_pda);
    println!(
        "Detailed Messasges Buffer PDA: {:?} ",
...
pub fn get_batch_pda(batch_number: u64) -> Result<()> {
    ...
    println!(
        "The PDA Id of the batch number {batch_number} is {:?}",
        batch_pda
    );
    Ok(())
}
