...
use tokens_gateway::{
    ...
    utils::address_derivation::{derive_native_token_vault_data, derive_spl_tokens_vault_data},
};
...
pub fn initialize_twine_solana_programs() -> Result<()> {
    ...
    twine_chain_instructions.extend(twine_chain_instruction::initialize_layer_zero_info(
        &account.pubkey(),
    ));
